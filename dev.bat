...
set "MANIM_PORT=8001"
if not defined TIMETABLE_SOLVER_TIMEOUT set "TIMETABLE_SOLVER_TIMEOUT=210"
if not defined TIMETABLE_SOLVER_SPENT_LIMIT set "TIMETABLE_SOLVER_SPENT_LIMIT=180s"
...
if defined PYTHON_CMD if exist "manim-service\main.py" (
    ...
    echo [Setup] Checking Manim Python environment...
    call node scripts\check-manim-env.js
    if errorlevel 1 (
        echo [WARN] Manim environment check failed. Manim will be disabled.
        set "MANIM_ENABLED=0"
    )
) else (
