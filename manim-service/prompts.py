...
【完整性强制约束】
1. **严禁省略任何代码**：禁止使用 "# ...", "# rest of code", "..." 或 "pass" 来代替已有代码。
2. **必须输出完整文件**：无论代码多长，你必须输出完整的、可直接运行的 Python 文件。
3. **禁止增量更新**：不要只输出修改的函数，必须输出包含所有 import 和类的完整代码。
...
【场景管理与布局策略】
1. **空间规划**：评估现有场景中的对象数量和大小，合理使用 Group 或缩放。
2. **转场动画设计**：使用 FadeOut/FadeIn 或 ReplacementTransform 进行优雅转场。
3. **边界检查与布局优化**：确保所有对象都在屏幕内 (Frame Width ~14, Height ~8)。
4. **多步骤动画序列**：将复杂的展示分解为多个步骤。
...
4. **文字布局规范**：文字标签必须与图形对象分层显示，避免任何重叠
5. **严禁在 MathTex 中使用中文**：所有中文必须使用 `Text()` 类。
6. **🌟 代码注释要求**：必须为每一段关键代码添加详细的中文注释。
...
【完整性强制约束】
**CRITICAL: You MUST output the FULL CODE.**
- 禁止使用省略号 (...) 或注释占位符 (# ... code unchanged)。
- 必须输出包含 import、类定义和 construct 方法的完整代码。
- 如果你偷懒省略了代码，会导致系统崩溃。
...
【改进策略】
根据分析器的报告，采用以下策略：
1. **PASS 评级** → 优化润色（变量命名、注释、微调参数）
2. **WARN 评级** → 针对性修复（布局、重叠、MathTex中文、import丢失）
3. **FAIL 评级** → 重新设计
