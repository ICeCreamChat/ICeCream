...
RULES: list[dict[str, str]] = [
    ...
    {
        "id": "stage_cleanup_missing",
        "title": "分镜状态清理",
        "generation": "每个阶段的对象必须用 VGroup 管理；切换阶段时用 FadeOut、ReplacementTransform、remove 或 clear 清理旧对象。",
        "critic": "多个阶段持续 self.add 而没有任何清理动作时应提示残影或堆叠风险。",
        "test": "连续添加多个阶段对象且没有 FadeOut/ReplacementTransform 应触发 stage_cleanup_missing。",
    },
    {
        "id": "connector_offscreen_risk",
        "title": "连线和箭头安全边距",
        "generation": "箭头、线段和连接器必须以可见对象端点为依据，并留在安全边距内；不要用大幅 shift 推到画面边缘。",
        "critic": "大幅 shift 或连接器贴边应静态失败或视觉失败。",
        "test": "Arrow/Line 大幅 shift 到边缘必须触发 connector_offscreen_risk 或 connector_offscreen。",
    },
    {
        "id": "panel_overlap_risk",
        "title": "面板防重叠",
        "generation": "多个说明面板必须放入 VGroup 后 arrange，或分别放入固定布局区；禁止公式弹窗覆盖主体卡片。",
        "critic": "多个 Rectangle/RoundedRectangle 面板没有统一排列时应提示重叠风险。",
        "test": "三个以上面板且没有 arrange/place_in_zone 应触发 panel_overlap_risk。",
    },
    {
        "id": "derivation_layout_missing",
        "title": "推导动画分区",
        "generation": "推导类动画必须分成定义区、推导区、结论区；推导公式不得覆盖原对象或步骤说明。",
        "critic": "推导/证明/过程类请求缺少分区布局时应提示 derivation_layout_missing。",
        "test": "推导 prompt 生成无分区布局代码时应触发 derivation_layout_missing。",
    },
    {
        "id": "canvas_quality",
        "title": "全画布浅色教学画面",
