...
test('seating planner exposes natural-language requirements and an editable SVG recognition result', async () => {
  ...
  assert.match(diagramSource, /groupsPerRow = 4/);
  assert.match(diagramSource, /groupSeparation = compact \? 7 : 14/);
  assert.match(diagramSource, /normalGap = compact \? 11 : 22/);
  assert.match(diagramSource, /walkwayGap = compact \? 18 : 34/);
  assert.match(diagramSource, /mainWalkwayGap = compact \? 26 : 42/);
  ...
  assert.match(diagramSource, /mainBoundaryIndex = Math\.floor\(groupsPerRow \/ 2\) - 1/);
  assert.match(diagramSource, /appendCrossMainAisle/);
  assert.match(diagramSource, /sp-arrangement-svg__main-aisle--cross/);
...
test('seating planner previews AI layout before confirming student assignment', async () => {
  ...
  assert.doesNotMatch(source, /sp-layout-preview-cancel/);
...
test('seating planner keeps preview facts and confirmation controls together above the canvas', async () => {
  ...
  assert.match(source, /布局预览图例/);
  assert.match(source, />间距</);
  assert.match(source, />过道</);
  assert.doesNotMatch(source, /id="sp-layout-preview-cancel"/);
  assert.match(source, /sp-rule-legend--walkway" data-lucide="person-standing/);
  assert.match(styles, /\.sp-rule-legend--gap::after/);
